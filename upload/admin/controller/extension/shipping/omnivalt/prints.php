 ...
 */
class ControllerExtensionShippingOmnivaltPrints extends Controller
{
	...
	private function updateOrderStatus($order_id, $order_status_id, $comment)
	{
		...
		$this->db->query(
			...
			SET `order_id` = '" . (int) $order_id . "', `order_status_id` = '" . (int) $order_status_id . "', `notify` = '0', `comment` = '" . $this->db->escape($comment) . "', `date_added` = NOW()
			"
		);
	}
	...
	private function readyStatus()
	{
		...
		if (!$status_id) {
			$query = $this->db->query(
				...
				VALUES (" . (int) $this->config->get('config_language_id') . ",'Ready for shipping')
				"
			);
	...
	private function getOrderTrack($order_id)
	{
		...
		if ($status_id) {
			$query = $this->db->query(
				...
				WHERE  `order_status_id` = " . (int) $status_id . " AND `order_id` = " . (int) $order_id . " 
	...
	public function manifest()
	{
		if (isset($_POST['selected']) && count($_POST['selected'])) {
			...
			$shop_addr = '<table cellspacing="0" cellpadding="1" border="0"><tr><td>' . date('Y-m-d H:i:s') . '</td><td>Siuntėjo adresas:<br/>' . $this->config->get('shipping_omnivalt_sender_name') . '<br/>' . $this->config->get('shipping_omnivalt_sender_address') . ', ' . $this->config->get('shipping_omnivalt_sender_postcode') . '<br/>' . $this->config->get('shipping_omnivalt_sender_city') . ', ' . $this->config->get('shipping_omnivalt_sender_country_code') . '<br/></td></tr></table>';
	...
	public function labels($generate = false)
	{
		...
		if (isset($_POST['selected']) && count($_POST['selected'])) {
			...
			$this->load->model('sale/order');
			require_once DIR_SYSTEM . 'library/omnivalt_lib/tcpdf/tcpdf.php';
			require_once DIR_SYSTEM . 'library/omnivalt_lib/fpdi/src/autoload.php';
			$errors = array();
			$object = '';
			$pages = 0;
			...
			foreach ($_POST['selected'] as $order_id) {
				...
				if ($order['shipping_code'] == 'omnivalt.courier') {
					...
					if ($shortage > 0 && is_integer($shortageLbl)) { //var_dump($shortageLbl);exit();
						$status = $this->omnivaltAPI->get_tracking_number($order, $weight, $shortage, 'courier');
						if ($status['status']) {
							foreach ($status['barcodes'] as $barcode) {
								$labelPrint = $order_id . '_' . $shortageLbl;
								$this->omnivaltAPI->getShipmentLabels($barcode, $labelPrint);
